...
// source key prefix to w/o leading but w/ trailing slash
const sourceKeyPrefix = process.env.SOURCE_KEY_PREFIX;
...
exports.handler = async (event, context, callback) => {
  const moves = event.Records.map(record => {
    ...
    if (match == null) {
      ...
      const filePrefix = sourceKey.replace(sourceKeyPrefix, '').replace(filename, '');
      //const targetKey = `${targetKeyPrefix}year=${year}/month=${month}/day=${day}/hour=${hour}/${filename}`;
      const targetKey = `${targetKeyPrefix}${filePrefix}year=${year}/month=${month}/day=${day}/hour=${hour}/${filename}`;
